...
python scripts/dump_attention.py --ckpt "$WG_CKPT" --dev-tsv "$DATASET_DIR/dev.tsv" --out-dir "$OUT_DIR_WG" --task "$TASK_ARG" --batch-size 32 $SAMPLES_ARG

echo "[2/4] Dumping attention for Random model..."
python scripts/dump_attention.py --ckpt "$RANDOM_CKPT" --dev-tsv "$DATASET_DIR/dev.tsv" --out-dir "$OUT_DIR_RANDOM" --task "$TASK_ARG" --batch-size 32 $SAMPLES_ARG

echo "[3/4] Finding motifs for WG model..."
python -m flashchrombert.legacy.find_motifs --data_dir "$DATASET_DIR" --predict_dir "$OUT_DIR_WG" --window_size 12 --min_len 5 --pval_cutoff 0.005 --min_n_motif 2 --align_all_ties --save_file_dir "$OUT_DIR_WG/result"

echo "[4/4] Finding motifs for Random model..."
python -m flashchrombert.legacy.find_motifs --data_dir "$DATASET_DIR" --predict_dir "$OUT_DIR_RANDOM" --window_size 12 --min_len 5 --pval_cutoff 0.005 --min_n_motif 2 --align_all_ties --save_file_dir "$OUT_DIR_RANDOM/result"
